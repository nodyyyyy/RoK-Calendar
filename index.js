...
/* ===== MULTI-GUILD CONFIG ===== */
const GUILD_MAIN   = '1323254900980711494'; // Original server
const GUILD_RUINS  = '1478855526015303720'; // New server (Ancient Ruins / Altar of Darkness)

/* ===== ANCIENT RUINS & ALTAR OF DARKNESS SCHEDULE (UTC) ===== */
// Format: [year, month (0-based), day, hour, minute]
const ANCIENT_RUINS_SCHEDULE = [
  [2026,  2, 18, 12,  0], // Wed 18.3
  [2026,  2, 20,  4,  0], // Fri 20.3
  [2026,  2, 21, 20,  0], // Sat 21.3
  [2026,  2, 23, 12,  0], // Mon 23.3
  [2026,  2, 25,  4,  0], // Wed 25.3
  [2026,  2, 26, 20,  0], // Thu 26.3
  [2026,  2, 28, 12,  0], // Sat 28.3
  [2026,  2, 30,  4,  0], // Mon 30.3
  [2026,  2, 31, 20,  0], // Tue 31.3
  [2026,  3,  2, 12,  0], // Thu  2.4
  [2026,  3,  4,  4,  0], // Sat  4.4
  [2026,  3,  5, 20,  0], // Sun  5.4
  [2026,  3,  7, 12,  0], // Tue  7.4
  [2026,  3,  9,  4,  0], // Thu  9.4
  [2026,  3, 10, 20,  0], // Fri 10.4
  [2026,  3, 12, 12,  0], // Sun 12.4
  [2026,  3, 14,  4,  0], // Tue 14.4
  [2026,  3, 15, 20,  0], // Wed 15.4
  [2026,  3, 17, 12,  0], // Fri 17.4
  [2026,  3, 19,  4,  0], // Sun 19.4
  [2026,  3, 20, 20,  0], // Mon 20.4
  [2026,  3, 22, 12,  0], // Wed 22.4
  [2026,  3, 24,  4,  0], // Fri 24.4
  [2026,  3, 25, 20,  0], // Sat 25.4
  [2026,  3, 27, 12,  0], // Mon 27.4
  [2026,  3, 29,  4,  0], // Wed 29.4
  [2026,  3, 30, 20,  0], // Thu 30.4
];

const ALTAR_OF_DARKNESS_SCHEDULE = [
  [2026,  3,  3, 12,  0], // Fri  3.4
  [2026,  3,  7,  2,  0], // Tue  7.4
  [2026,  3, 10, 16,  0], // Fri 10.4
  [2026,  3, 14,  6,  0], // Tue 14.4
  [2026,  3, 17, 20,  0], // Fri 17.4
  [2026,  3, 21, 10,  0], // Tue 21.4
  [2026,  3, 25,  0,  0], // Sat 25.4
  [2026,  3, 28, 14,  0], // Tue 28.4
];

// In-memory storage: { channelId: string } per guild+category
const ruinsReminderChannels = {
  ancientRuins:    null, // channel ID for Ancient Ruins reminders
  altarOfDarkness: null, // channel ID for Altar of Darkness reminders
};

// Track last sent reminder message IDs so we can delete them before the next one
const ruinsLastMsgId = {
  ancientRuins:    null,
  altarOfDarkness: null,
};
...
async function initDB() {
  ...
  `);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ruins_reminder_config (
...
client.once('clientReady', async () => {
  ...
  // Restore ruins reminder config from DB
  try {
    const { rows } = await pool.query(`SELECT category, channel_id, last_msg_id FROM ruins_reminder_config`);
    for (const r of rows) {
      if (r.category === 'ancientRuins')    { ruinsReminderChannels.ancientRuins    = r.channel_id; ruinsLastMsgId.ancientRuins    = r.last_msg_id; }
      if (r.category === 'altarOfDarkness') { ruinsReminderChannels.altarOfDarkness = r.channel_id; ruinsLastMsgId.altarOfDarkness = r.last_msg_id; }
    }
    console.log('🏛️ Ruins reminder config restored from DB.');
  } catch (err) {
    console.error('Failed to restore ruins reminder config:', err.message);
  }

  startRuinsReminderChecker();
});
...
const ruinsCommands = [
  new SlashCommandBuilder()
    .setName('set_ruins_reminder')
    .setDescription('(Admin) Set this channel as the reminder channel for Ancient Ruins & Altar of Darkness'),
].map(c => c.toJSON());
...
(async () => {
  try {
    // Commands for original server
    await rest.put(
      Routes.applicationGuildCommands(CLIENT_ID, GUILD_MAIN),
      { body: commands }
    );
    // Commands for ruins server
    await rest.put(
      Routes.applicationGuildCommands(CLIENT_ID, GUILD_RUINS),
      { body: ruinsCommands }
    );
    console.log('✅ Slash commands registered for both guilds.');
  } catch (error) {
    console.error(error);
  }
})();
...
client.on('interactionCreate', async interaction => {
  ...
  /* ===== GUILD ROUTING ===== */
  // New server: only handle set_ruins_reminder command
  if (interaction.guildId === GUILD_RUINS) {
    if (interaction.isChatInputCommand() && interaction.commandName === 'set_ruins_reminder') {
      if (!interaction.member.permissions.has('Administrator')) {
        return interaction.reply({ content: '❌ Only administrators can use this command.', ephemeral: true });
      }

      const channelId = interaction.channelId;
      ruinsReminderChannels.ancientRuins    = channelId;
      ruinsReminderChannels.altarOfDarkness = channelId;

      // Persist to DB
      await pool.query(
        `INSERT INTO ruins_reminder_config (category, channel_id) VALUES ('ancientRuins', $1)
         ON CONFLICT (category) DO UPDATE SET channel_id = $1`,
        [channelId]
      ).catch(err => console.error('ruins config save error:', err.message));
      await pool.query(
        `INSERT INTO ruins_reminder_config (category, channel_id) VALUES ('altarOfDarkness', $1)
         ON CONFLICT (category) DO UPDATE SET channel_id = $1`,
        [channelId]
      ).catch(err => console.error('ruins config save error:', err.message));

      return interaction.reply({
        content: `✅ **Reminder channel set!**\nThis channel will receive reminders **1 hour before** each **Ancient Ruins** and **Altar of Darkness** event.`,
        ephemeral: true
      });
    }
    return; // Ignore all other interactions from ruins guild
  }

  // Original server: block if not from main guild
  if (interaction.guildId !== GUILD_MAIN) return;
...
async function runDailyAnnouncement() {
  try {
    ...
    // Filter out already announced events
    const newEvents = [];
    ...
    // Get all channels that have a live_events embed
    ...
    if (!hasEvents) {
      embed.setTitle('📅  No New Events Tomorrow');
      embed.setDescription('There are no new events starting tomorrow.');
    } else if (count === 1) {
      const event = newEvents[0];
      const start = new Date(event.start.dateTime || event.start.date);
      const unix = Math.floor(start.getTime() / 1000);
      const emoji = getEventEmoji(event.summary);
      embed.setDescription('📅 **Upcoming Tomorrow**');
    ...
    const msgContent = hasEvents
      ? `<@&${ANNOUNCE_ROLE_ID}> — ${count === 1 ? 'New event' : `${count} new events`} starting tomorrow!`
      : `<@&${ANNOUNCE_ROLE_ID}> — Daily update`;
    ...
    for (const row of channels) {
      try {
        ...
        const { rows: prev } = await pool.query(
          `SELECT message_id FROM announcement_messages WHERE channel_id = $1`, [row.channel_id]
        );
        if (prev.length > 0) {
          try {
            const oldMsg = await channel.messages.fetch(prev[0].message_id);
            await oldMsg.delete();
          } catch { /* message may already be deleted */ }
        }

        // Send new announcement
        const newMsg = await channel.send({ content: msgContent, embeds: [embed] });

        // Save new message_id
        await pool.query(
          `INSERT INTO announcement_messages (channel_id, message_id) VALUES ($1, $2)
           ON CONFLICT (channel_id) DO UPDATE SET message_id = $2`,
          [row.channel_id, newMsg.id]
        );
    ...
    console.log(`📅 Announcement sent. New events: ${count}`);
...
/* ===== ANCIENT RUINS / ALTAR OF DARKNESS REMINDER CHECKER ===== */

function startRuinsReminderChecker() {
  // Track which events have already been reminded (by their UTC ISO string)
  const reminded = new Set();

  setInterval(async () => {
    const now = new Date();

    async function checkSchedule(category, schedule, title, color) {
      const channelId = ruinsReminderChannels[category];
      if (!channelId) return;

      for (const [y, mo, d, h, m] of schedule) {
        const eventTime = new Date(Date.UTC(y, mo, d, h, m, 0));
        const reminderTime = new Date(eventTime.getTime() - 60 * 60 * 1000); // 1 hour before
        const key = `${category}_${eventTime.toISOString()}`;

        if (reminded.has(key)) continue;
        if (now < reminderTime || now >= eventTime) continue;
        ...
        try {
          ...
          // Delete previous reminder message for this category
          const lastMsgId = ruinsLastMsgId[category];
          if (lastMsgId) {
            try {
          ...
          const embed = new EmbedBuilder()
            .setColor(color)
            .setTitle(`⏰  ${title}`)
            .setDescription(`The ${category === 'ancientRuins' ? 'Ancient Ruins' : 'Altar of Darkness'} will open in **1 hour**!\n\n📅 <t:${unix}:F>\n🕐 <t:${unix}:R>`)
            .setFooter({ text: 'Time in UTC' })
            .setTimestamp();
          ...
          // Save new message ID
          ruinsLastMsgId[category] = sent.id;
          await pool.query(
            `UPDATE ruins_reminder_config SET last_msg_id = $1 WHERE category = $2`,
            [sent.id, category]
          ).catch(err => console.error('ruins last_msg_id save error:', err.message));

          console.log(`🏛️ Sent ${category} reminder for ${eventTime.toISOString()}`);
        } catch (err) {
          console.error(`Ruins reminder send error (${category}):`, err.message);
        }
      }
    }

    await checkSchedule('ancientRuins',    ANCIENT_RUINS_SCHEDULE,    'Ancient Ruins',    '#2ecc71');
    await checkSchedule('altarOfDarkness', ALTAR_OF_DARKNESS_SCHEDULE, 'Altar of Darkness', '#8e44ad');

  }, 30000); // check every 30 seconds
}

/* ===== END ANCIENT RUINS / ALTAR OF DARKNESS REMINDER CHECKER ===== */
