...
async function buildLiveEventsEmbed() {
  ...
  const embed = new EmbedBuilder()
    .setColor("#7B2CBF")
    .setTitle("📅  Kingdom 3558 — Weekly Events")
    .setDescription(`**Week of ${headerFormatter.format(startOfWeek)} – ${headerFormatter.format(endOfWeek)}**\n━━━━━━━━━━━━━━━━━━━━━━━━\n🟣 Upcoming  ·  🟢 Today  ·  🔵 In Progress`)
...
async function runDailyAnnouncement() {
  try {
    ...
    // Filter out events already announced
    const newEvents = [];
    for (const event of tomorrowEvents) {
      const eventId = event.id;
      const { rows } = await pool.query(
        `SELECT event_id FROM announced_events WHERE event_id = $1`, [eventId]
      );
    ...
    // Send to all live_events channels
    ...
    // Mark all new events as announced
    for (const event of newEvents) {
      await pool.query(
...
function scheduleDailyAnnouncement() {
  ...
  console.log(`📅 Daily announcement scheduled in ${Math.round(msUntilNext / 1000 / 60)} minutes.`);
