...
await pool.query(`CREATE TABLE IF NOT EXISTS events ( message_id TEXT PRIMARY KEY, channel_id TEXT, event_name TEXT, event_time TIMESTAMPTZ, reminder_sent INTEGER DEFAULT 0 )`);
await pool.query(`CREATE TABLE IF NOT EXISTS signups ( message_id TEXT, user_id TEXT, status TEXT CHECK(status IN ('yes','no')), PRIMARY KEY (message_id, user_id) )`);
await pool.query(`CREATE TABLE IF NOT EXISTS reminder_events ( id SERIAL PRIMARY KEY, channel_id TEXT, role_id TEXT, event_name TEXT, event_time TIMESTAMPTZ, reminder_minutes INTEGER, reminder_sent INTEGER DEFAULT 0 )`);
await pool.query(`CREATE TABLE IF NOT EXISTS live_event_messages ( id SERIAL PRIMARY KEY, channel_id TEXT NOT NULL, message_id TEXT NOT NULL UNIQUE )`);
...
`${statusBadge}  ·  ➤ ${relativeText} ┣ 📆  ${startDate}  →  ${endDate} ┗ ⏳  ${durationDays} day${durationDays > 1 ? "s" : ""} \u200b`,
inline: false
});
